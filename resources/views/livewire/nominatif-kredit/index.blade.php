<div class="container-fluid px-0 nominatif-report">
  ...
  <style>
    ...
    /* NOA Kredit/Tabungan/Deposito: scroll vertikal + header sticky. */
    ...
    .noa-table-scroll .report-table thead th{
    ...
    <div class="row g-3 align-items-end">
      ...
      <div class="col-12 col-lg-2">
        ...
        <div class="filter-control">
          ...
          <i class="bi bi-arrow-left-right filter-control-icon"></i>

          <select class="filter-input" wire:model.live="filterDataMode">

            <option value="pengajuan">By Tgl Prospek</option>

            <option value="realisasi">By Tgl Realisasi</option>
      ...
      <div class="col-12 col-lg-2">
        ...
        <div class="filter-control">
          ...
          <select class="filter-input"
                  ...
                  wire:model.live="filterDateMode"

                  data-date-mode-select>
      ...
      {{--
        ...
        FIX UX: kedua kelompok filter selalu ada di DOM.

        JavaScript menampilkan/menyembunyikannya langsung saat Mode Tanggal berubah,

        jadi tidak perlu menunggu round-trip Livewire + query dashboard.

      --}}

      <div class="col-6 col-lg-2"

           data-date-mode-group="monthly"

           style="{{ $filterDateMode === 'monthly' ? '' : 'display:none;' }}">
      ...
      <div class="col-6 col-lg-2"
           ...
           data-date-mode-group="range"

           style="{{ $filterDateMode === 'range' ? '' : 'display:none;' }}">
      ...
      <div class="rank-box">
        ...
        <div class="mt-3 d-flex flex-wrap gap-2">
          ...
          <span class="badge bg-primary rounded-pill px-3 py-2">
            ...
            @if($filterDataMode === 'realisasi')

              Tidak dihitung pada mode realisasi
      ...
      <div class="tab-content" id="noaRealisasiTabContent">
        ...
        @foreach(['KREDIT' => 'Kredit', 'TABUNGAN' => 'Tabungan', 'DEPOSITO' => 'Deposito'] as $key => $label)
          ...
          <div class="tab-pane fade {{ $key === 'KREDIT' ? 'show active' : '' }}"
            ...
            <div class="table-responsive noa-table-scroll">
                ...
                <tbody>
                  ...
                  @forelse($productRows as $index => $row)
                    ...
                    <tr>
                      ...
                      <td>{{ $tanggalView($row['tanggal_prospek'] ?? null) }}</td>

                      <td>{{ $tanggalView($row['tanggal_realisasi'] ?? null) }}</td>

                      <td class="fw-bold">{{ $row['kode_cabang'] }} - {{ $row['nama_cabang'] }}</td>

                      <td class="fw-bold">{{ $row['nama_nasabah'] ?: '-' }}</td>
              ...
              <tr>
                ...
                <td colspan="6">
                  ...
                  <div class="empty-state">
                    ...
                    Data belum match tidak ditampilkan pada mode Tanggal Realisasi karena data tersebut belum memiliki tgl_realisasi di tabel nominatif.
  ...
  <script>
    ...
    (function () {
      ...
      if (window.__nominatifDateModeInstantBound) return;

      window.__nominatifDateModeInstantBound = true;

      function syncDateModeFields(select) {

        if (!select) return;

        var shell = select.closest('.pro-filter-shell') || document;

        var mode = select.value || 'all';

        shell.querySelectorAll('[data-date-mode-group]').forEach(function (group) {

          group.style.display = group.getAttribute('data-date-mode-group') === mode ? '' : 'none';

        });

      }

      function syncCurrentDateMode() {

        document.querySelectorAll('[data-date-mode-select]').forEach(syncDateModeFields);

      }

      // Delegated event: tetap bekerja walaupun elemen diganti/morph oleh Livewire.

      document.addEventListener('change', function (event) {

        var select = event.target.closest && event.target.closest('[data-date-mode-select]');

        if (select) syncDateModeFields(select);

      }, true);

      // Beberapa searchable-select mengirim event input sebelum change.
      ...
      document.addEventListener('DOMContentLoaded', syncCurrentDateMode);

      document.addEventListener('livewire:navigated', syncCurrentDateMode);
      ...
      window.addEventListener('load', syncCurrentDateMode);
